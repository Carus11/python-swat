...
#       label = 'Selected Rows from Table DATASOURCES.CARS_SINGLE'
#       thstr = ['', 'Make', 'Model', 'Type', 'Origin', 'DriveTrain', 'MSRP', 'Invoice',
#                'Engine Size (L)', 'Cylinders', 'Horsepower', 'MPG (City)', 'MPG (Highway)',
#                'Weight (LBS)', 'Wheelbase (IN)', 'Length (IN)']

#       jsdata = re.search(r"new swat\.SASDataFrame\(\$\('#[^']+?'\), JSON\.parse\('(.+?)'\)\);", 
#                          js, re.S).group(1)
...
#       swat.options.repr_javascript = False
    ...
    def test_reshape_bygroups(self):
        out = self.table.groupby(['Origin', 'Cylinders']).summary(subset=['Min', 'Max', 'Mean'])        
        ...
        self.assertEqual(list(both.index.names), ['Origin', 'Origin_f', 'Cylinders', 'Cylinders_f'])
        ...
        self.assertEqual(list(both2.index.names), ['Origin', 'Origin_f', 'Cylinders', 'Cylinders_f'])
        ...
        self.assertEqual(list(out.index.names), [None])
        self.assertEqual(list(out.columns), ['Origin', 'Origin_f', 'Cylinders', 'Cylinders_f'] + columns)
        self.assertEqual(out.Origin[0], 'Asia')
        ...
        out = reshape_bygroups(fmt, bygroup_columns='both', bygroup_as_index=False, bygroup_formatted_suffix='.1')
        self.assertEqual(list(out.index.names), [None])
        self.assertEqual(list(out.columns), ['Origin', 'Origin.1', 'Cylinders', 'Cylinders.1'] + columns)
        self.assertEqual(out.Origin[0], 'Asia')
